 ...
 *   - Else if blogs.posting_frequency_days is set:
 *     lastPostVerifiedAt must be >= that many days ago.
...
export async function runAutoPublishCron(): Promise<AutoPublishResult> {
  ...
  // for niche + total cap. Cadence comes from either:
  //   - blogs.posting_frequency (varchar, parsed as posts/day), OR
  //   - blogs.posting_frequency_days (integer, every N days)
  const rows = await db
    .select({
      blog: blogs,
      clientNiche: clients.niche,
  ...
  // Per-blog count of posts already published today (for daily cap).
